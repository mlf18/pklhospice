...
//direktori gambar
  ...
  unlink($vfile_upload);
	...
	$sql="INSERT INTO `artikel` (`id`, `nama`, `deskripsi`, `gambar`,`tanggal`) VALUES ('', '$judul', '$isi', '$ima',now())";
	$result=$db->query($sql);
	if(!$result){
		echo'
		<div class="alert alert-danger">
			<strong>Error!</strong> <a href="body.php?modul=9&aksi=tambah">Try Again</a>
		</div>';
		die("Could not query the database: </br>".$db->error);
	}
	echo'
	<div class="alert alert-success">
		<strong>Success!</strong> <a href="body.php?modul=9">Kembali</a>
	</div>';
	}
?>
...
?>
<?php function lihat_artikel(){ 
global $db;?>
<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>Artikel</h2>
						<div class="box-icon">
					...
					<?php							
						$sql= "SELECT id,nama,tanggal FROM artikel order by id desc";
						$q= $db->query($sql);
					?>
					
					<div class="box-content">
					<a href="body.php?modul=9&aksi=tambah" class="btn btn-primary"><i class="halflings-icon white plus-sign"></i>Tambah Artikel</a>
					...
					<tbody>
						...
						?>
							...
							<tr>
								...
								<td class="center">
									<?php echo '<a class="btn btn-info" href="body.php?modul=9&aksi=edit&id='.$row->id.'">'; ?>
										<i class="halflings-icon white edit"></i>
									</a>
									<?php echo '<a class="btn btn-danger" href="body.php?modul=9&aksi=hapus&id='.$row->id.'"onclick="return confirm (\'Apakah Anda yakin ingin menghapus data?\');" >';?>
...
//direktori gambar
  ...
  unlink($vfile_upload);
	imagedestroy($im2);}
	$sql="UPDATE `artikel` SET `nama` = '$judul',`deskripsi`='$isi',`gambar`='$ima',`tanggal`=now() WHERE `artikel`.`id` = '$id'";
	$result=$db->query($sql);
	if(!$result){
		echo'
		<div class="alert alert-danger">
			<strong>Error!</strong> <a href="body.php?modul=9&aksi=edit">Try Again</a>
		</div>';
		die("Could not query the database: </br>".$db->error);
	}
	echo'
	...
	<div class="box-content">	
			<form class="form-horizontal" name="postform" action="" method="post" enctype='multipart/form-data'>
				...
				<input type="hidden" id="id" name="id" value="<?php echo $idp;?>">
				  ...
				  <div class="form-actions">
					<button type="submit" class="btn btn-primary">Save changes</button>
					<a class="btn" href="body.php?modul=9">Cancel</a>
				  </div>
				</fieldset>
			  </form>
		</div> <?php } 
